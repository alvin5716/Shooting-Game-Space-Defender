...
Enemy_2_Red::Enemy_2_Red(Player* player, int health, int radius, int shoot_cd, int shoot_cd_init, double x, double y, double xv, double yv, double xa, double ya, bool bounceable, bool stopable)
...
std::vector<Bullet*>* Enemy_2_Red::shoot() {
    if(shoot_timer>=shoot_cd && (shoot_timer-shoot_cd)%3==0) {
        double bullet_v, bullet_a, cos, sin;
        int t;
        std::vector<Bullet*>* new_bullets=new std::vector<Bullet*>;
        Bullet* new_bullet;
        //bullet v, a and count
        t = (shoot_timer-shoot_cd)/3;
        bullet_v = 4;
        bullet_a = 0;
