...
Enemy_Yellow::Enemy_Yellow(Player* player, int health, int radius, int shoot_cd, int shoot_cd_init, double x, double y, double xv, double yv, double xa, double ya, bool bounceable, bool stopable)
    :Enemy(ImgPixmap::Level1::enemy_1_yellow,40,37,std::round(radius*2.162),radius*2,player,health,radius,shoot_cd,shoot_cd_init,x,y,xv,yv,xa,ya,bounceable,stopable)
{
    point=5;
}
...
std::vector<Bullet*>* Enemy_Yellow::shoot() {
    if(shoot_timer>=shoot_cd && (shoot_timer-shoot_cd)%13==0) {
        ...
        for(int i=-(bullet_count/2);i<=(bullet_count/2-1);++i) {
            ...
            new_bullet = new Bullet(ImgPixmap::Level1::bullet_1_yellow,16,x,y,bullet_v*cos,bullet_v*sin,bullet_a*cos,bullet_a*sin);
