...
:Enemy_4(QString(":/res/enemy19.png"),199,153,std::round(3.902*radius),3*radius,player,health,radius,shoot_cd,shoot_cd_init,x,y,xv,yv,xa,ya,true,stopable)
{
    move_speed=1.6;
...
void Enemy_4_Pink::skill() {
    if(already_enter) {
        if(x<0+radius || x>Game::FrameWidth-radius || y<0+radius || y>Game::FrameHeight-radius) {
            setAcceleration(0,0);
            setSpeed(0,0);
            Enemy* new_enemy = new Enemy_Temp(Enemy_Temp::enemy_4_pink_shoot,player,shoot_cd,shoot_cd_init,x,y);
