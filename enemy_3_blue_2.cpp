...
void Enemy_3_Blue_2::skill() {
    ...
    [this](){
        ...
        if(skill_timer>=200) {
            if(room==nullptr) {
                room = new Effect(":/res/effect/magic_red.png",120,120,room_radius*2,room_radius*2,-1,player->getX(),player->getY());
                room->setOpacity(0.6);
                room->setZValue(-2);
                room->rotateStart();
                room_radius_rate=0;
    ...
    if(shoot_timer>=shoot_cd && (shoot_timer-shoot_cd)%interval==0) {
        ...
        //bullet faster
            ...
            for(int i=-(bullet_count/2);i<=(bullet_count/2-((bullet_count%2==0)?1:0));++i) {
                ...
                cos = std::cos(angle+i*2*M_PI/bullet_count);
                Bullet_Distance* new_bullet_distance;
                new_bullet = new_bullet_distance = new Bullet_Distance(":/res/bullet/1/black.png",bullet_radius,player,room_radius,shootXPos(),shootYPos(),bullet_v*cos,bullet_v*sin,bullet_a*cos,bullet_a*sin);
